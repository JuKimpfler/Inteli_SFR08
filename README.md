...
| Crosstalk bei mehreren Sensoren | SRF08Manager: paarweises Feuern (2 gleichzeitig) |
...
Der **SRF08Manager** orchestriert mehrere Sensoren paarweise:

```
Paar A (0+2): startRanging() → RANGING ──► beide fertig → IDLE
                                             │
                                             └──► Paar B (1+3): startRanging() → RANGING
                                                            beide fertig → IDLE
                                                               │
                                                               └──► Paar A (0+2): ...
```
...
Der `SRF08Manager` feuert immer zwei Sensoren gleichzeitig.
Bei vier Sensoren nutzt er feste Gegenüber-Paare:
- `0 + 2` (z. B. vorne + hinten)
- `1 + 3` (z. B. links + rechts)
...
- Beide Sensoren liefern etwa alle **14 ms** neue Daten (~70 Hz)

Bei 4 Sensoren:
- Paar `0+2` misst in Slot A (~14 ms)
- Paar `1+3` misst in Slot B (~14 ms)
- Vollzyklus: ~28 ms (~36 Hz pro Sensor)
...
- Sicherstellen dass **SRF08Manager** verwendet wird (paarweises Feuern)
